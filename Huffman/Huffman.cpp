...
nodeLista* novoNodeLista(nodeArvore* nArv)
{
    // Aloca memória
    nodeLista* novo;
    if ((novo = (nodeLista*)malloc(sizeof(*novo))) == NULL) return NULL;
...
nodeArvore* novoNodeArvore(byte c, int frequencia, nodeArvore* esquerda, nodeArvore* direita)
{
    ...
    if ((novo = (nodeArvore*)malloc(sizeof(*novo))) == NULL) return NULL;
...
void FreeHuffmanTree(nodeArvore* n)
{
    ...
    else
    {
        ...
        std::cout << "\nTABLE_SYMBOL_LEFT__NODE:: " << direita;
        std::cout << "\nTABLE_SYMBOL_RIGHT_NODE:: " << esquerda;
    }
}
